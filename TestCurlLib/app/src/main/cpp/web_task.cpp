...
bool WebTask::sUseDynamicAry = false;
...
static size_t web_write_mem(void *ptr, size_t size, size_t nmemb, void *data) {
    ...
    if (WebTask::sUseDynamicAry){
        DynamicArray* ptrDynamicAry = (DynamicArray*)data;
        ptrDynamicAry->strcat((char*)ptr, size * nmemb);
    }
    else{
...
int WebTask::DoGetString() {
    ...
    curl_easy_setopt(m_curl, CURLOPT_WRITEFUNCTION,
                     web_write_mem); //指定由 web_write_mem 处理网络数据和本地内存数据

    /* we want the headers be written to this file handle */
    curl_easy_setopt(m_curl, CURLOPT_WRITEHEADER, NULL);
    if (sUseDynamicAry){
        curl_easy_setopt(m_curl, CURLOPT_WRITEDATA,
                         &mDynamicArySaveCont); //指定数据 和 &m_web_buf 对接，具体对接细节在 web_write_mem 中实现
    }
    else{
        ...
        curl_easy_setopt(m_curl, CURLOPT_WRITEDATA,
                         &m_web_buf); //指定数据 和 &m_web_buf 对接，具体对接细节在 web_write_mem 中实现
    }
...
DynamicArray& WebTask::GetDynamicArySaveCont(){
    return this->mDynamicArySaveCont;
}
